...
watch([() => props.pros, () => props.cons], () => {
  processMath()
}, { deep: true })
</script>

<template>
  <div ref="containerRef" grid="~ cols-2 gap-x-4 items-end" my4 absolute bottom-0 left-15 right-15>
    <div v-click flex="~ col gap-1" class="pros-cons-drawer">
      <div flex="~ gap-1 items-center" font-600 class="text-green-600 dark:text-green-400">
      ...
      <div
        ...
        px3 py3 pb-12 mb--10 relative
        flex="~ col gap-2"
      >
        <div v-for="pro in pros" :key="pro" flex="~ gap-2 items-center">
          <div i-ph-check-circle-duotone class="text-green-600 dark:text-green-400" text-xl flex-shrink-0 />
          {{ pro }}
        </div>
        <div absolute top--4px right-4px left-4px h-4px class="bg-green-100/70 dark:bg-green-800/30 border-t border-l border-r border-green-200/40 dark:border-green-600 rounded-t-lg backdrop-blur-sm" />
      </div>
    </div>

    <div v-click flex="~ col gap-1" class="pros-cons-drawer">
      <div flex="~ gap-1 items-center" class="text-red-600 dark:text-red-400" font-600>
      ...
      >
        <div v-for="con in cons" :key="con" flex="~ gap-2 items-center">
          <div i-ph-warning-duotone class="text-orange-600 dark:text-orange-400" text-xl flex-shrink-0 />
          {{ con }}
        </div>
        <div absolute top--4px right-4px left-4px h-4px class="bg-red-100/70 dark:bg-red-800/30 border-t border-l border-r border-red-200/40 dark:border-red-600 rounded-t-lg backdrop-blur-sm" />
      </div>
    </div>
  </div>
</template>
