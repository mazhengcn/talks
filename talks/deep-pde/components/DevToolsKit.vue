...
const isVue = computed(
  () => isAstro.value || $clicks.value === PRESENT_START + 1,
);
const isReact = computed(
  () => isAstro.value || $clicks.value === PRESENT_START + 2,
);
const isSvelte = computed(
  () => isAstro.value || $clicks.value === PRESENT_START + 3,
);
const isSolid = computed(
  () => isAstro.value || $clicks.value === PRESENT_START + 4,
);

const isFaded = computed(
  () => PRESENT_START <= $clicks.value && $clicks.value <= PRESENT_START + 5,
);
</script>
...
<template>
  <div relative of-hidden h-1em text-5xl>
    <div transition-all duration-500 :style="sliderStyle">
      ...
      <div font-700 flex="~ gap-2">
        ...
        <div text-green>Nuxt</div>
        DevTools
      </div>
      <div font-700 flex="~ gap-2">
        <div i-logos-vue />
        <div text-green>Vue</div>
        DevTools
      </div>
      <div font-700 flex="~ gap-2">
        <div i-logos-react />
        <div text-blue>React</div>
        DevTools?
      </div>
      <div font-700 flex="~ gap-2">
        <div i-logos-svelte-icon />
        <div text-red>Svelte</div>
        DevTools?
      </div>
      <div font-700 flex="~ gap-2">
        <div i-logos-solidjs-icon />
        <div text-blue>Solid</div>
        DevTools?
      </div>
      <div font-700 flex="~ gap-2">
        <div i-logos-astro-icon invert hue-rotate-180 />
        <div text-orange6>Astro</div>
        DevTools?
      </div>
      <div font-700 flex="~ gap-2">
        <div i-twemoji-winking-face />
        <div text-yellow>Your</div>
        DevTools?
      </div>
    </div>
  </div>

  <div v-click op50 my2>模块化、可组合、互相协作</div>
  ...
  <div flex="~ gap-2" mt2>
    ...
    <DevToolsModule
      v-after
      icon="i-carbon-volume-file-storage"
      name="文件路由"
      theme="teal"
      logo="i-logos-nuxt-icon"
      :class="{ op30: isFaded && !isNuxt }"
  ...
  <div flex="~ gap-2" mt2>
    ...
    <DevToolsModule
      v-click
      icon="i-carbon-heat-map"
      name="分包"
      theme="purple"
      logo="i-logos-qwik-icon"
      :class="{ op30: isFaded }"
    />
    <DevToolsModule
      v-click
      ...
      name="数据管理"
      theme="orange"
      logo="i-logos-astro-icon invert hue-rotate-180"
