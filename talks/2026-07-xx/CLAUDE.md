...
The visual design follows the **SJTU Visual Identity System** (`vi.sjtu.edu.cn`): clean white/neutral canvas, SJTU Red (#C8161E) primary brand color, system sans-serif/serif Chinese fonts, and JetBrains Mono code. This replaces the previous Claude.com DESIGN.md theme; typography and component structure are preserved, only theme color tokens changed.

## Design system reference

The `claude.design.md` file is the **previous** design token specification (Claude.com visual language) — kept for reference but no longer the active theme. The talk's UnoCSS config (`unocss.config.ts`) now implements SJTU VI colors modeled after the reference slide deck (百廿红) and the SJTU-2021 PowerPoint color scheme.

## Custom UnoCSS layer

`unocss.config.ts` defines the entire visual system beyond what Slidev's default preset provides:

- **Color scales:** `neutral` (clean grays 50–1000), `red` (SJTU Red primary, 50–950, with #C8161E at 500), `gold` (SJTU Gold accent, 50–950, with #FFC000 at 500)
- **Type scale shortcuts:** `text-display-{xl,lg,md,sm}` (serif), `text-title-{lg,md,sm}`, `text-body-{md,sm}`, `text-caption-token`, `text-caption-caps`
...
- **Web fonts:** JetBrains Mono only (Chinese fonts are system fonts — no web font download needed)

`styles/style.css` sets the typographic hierarchy: serif for `h1`/`h2` (weight 700), sans for `h3`–`h6` (weight 600), sans for body (weight 400). It also styles code blocks as dark-surface code-window cards, SJTU-red-tinted inline code, and KaTeX math.
...
- **`DataTable.vue`** — Card-wrapped table. Props: `headers`, `rows`, `highlightCol` (0-indexed column to highlight in SJTU red), `caption`, `enableLatex` (default true). Renders inline markdown and KaTeX in all cells.
